...
export const usePlugins = () => {
  ...
  // Plugin operations
  const installPluginFromGitHub = useCallback(async (request: LoadPluginFromGitHubRequest) => {
  ...
  const installPluginFromFile = useCallback(async (request: LoadPluginFromFileRequest) => {
  ...
  // Helper functions
  const getPluginByType = useCallback((type: 'cluster' | 'monitoring' | 'backup') => {
  ...
  const getPluginHealth = useCallback((pluginId: string) => {
